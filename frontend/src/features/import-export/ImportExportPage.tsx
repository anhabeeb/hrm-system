...
const formatJobFileType = (fileType?: string) => fileType === "json" || fileType === "csv" ? `${fileType.toUpperCase()} (Legacy)` : fileType?.toUpperCase() ?? "Not generated";
...
const ExportCreateDialog = ({ open, loading, error, visibleExportTypes, onOpenChange, onSubmit }: { open: boolean; loading?: boolean; error?: string | null; visibleExportTypes: string[]; onOpenChange: (open: boolean) => void; onSubmit: (payload: ExportCreatePayload) => void }) => {
  ...
  useEffect(() => {
    if (!open) {
      setPayload({ export_type: "employees", format: "xlsx", filters: {} });
      setLocalError(null);
    }
  }, [open]);
  useEffect(() => {
...
const ImportUploadDialog = ({ open, loading, error, onOpenChange, onSubmit }: { open: boolean; loading?: boolean; error?: string | null; onOpenChange: (open: boolean) => void; onSubmit: (payload: ImportUploadPayload) => void }) => {
  ...
  const submit = () => {
    ...
    if (!file.name.toLowerCase().endsWith(".xlsx") || file.type !== excelMime || dangerousTypes.has(file.type)) return setLocalError("Please upload an Excel .xlsx import file.");
    const reader = new FileReader();
    reader.onload = () => {
      const result = String(reader.result ?? "");
      onSubmit({ import_type: importType, file_type: "xlsx", file_name: file.name, mime_type: file.type, content_base64: result.split(",")[1] ?? "", reason: reason.trim() });
      setFile(null);
      setReason("");
      setLocalError(null);
      setFileInputKey((key) => key + 1);
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Upload import file</DialogTitle><DialogDescription>Upload an Excel file for validation before applying import changes.</DialogDescription></DialogHeader><Label className="space-y-1 text-sm">Import type<Select value={importType} onValueChange={setImportType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{importTypes.map((type) => <SelectItem key={type} value={type}>{formatJobType(type)}</SelectItem>)}</SelectContent></Select></Label><Label className="space-y-1 text-sm">Import file<Input key={fileInputKey} type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></Label><Textarea placeholder="Reason" value={reason} onChange={(event) => setReason(event.target.value)} />{localError || error ? <InlineAlert title={localError ?? error ?? ""} variant="error" /> : null}<DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><LoadingButton loading={loading} onClick={submit}>Upload import</LoadingButton></DialogFooter></DialogContent></Dialog>;
};
...
export const ImportExportPage = () => {
  ...
  const reasonMutation = useMutation({
    ...
    onSuccess: async (response) => {
      if (reasonAction === "applyImport") {
        const failedRows = Number("failed_rows" in response ? response.failed_rows ?? 0 : 0);
        const appliedRows = Number("applied_rows" in response ? response.applied_rows ?? 0 : 0);
        setSuccessMessage(failedRows > 0 ? `Import applied with ${appliedRows} row(s) saved and ${failedRows} row-level issue(s).` : "Import applied successfully.");
      } else {
        setSuccessMessage(reasonAction === "validateImport" ? "Import validation completed successfully." : reasonAction === "cancelImport" ? "Import job cancelled successfully." : reasonAction === "retryExport" ? "Export job retry requested successfully." : "Export job cancelled successfully.");
      }

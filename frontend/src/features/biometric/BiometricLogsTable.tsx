...
export const BiometricLogsTable = ({ rows, loading, pagination, canReprocess, onView, onReprocess, onPageChange, onPageSizeChange }: { rows: BiometricLog[]; loading?: boolean; pagination?: Pagination; canReprocess: boolean; onView: (row: BiometricLog) => void; onReprocess: (row: BiometricLog) => void; onPageChange: (page: number) => void; onPageSizeChange: (pageSize: number) => void }) => (
  <DataTable columns={columns} rows={rows} getRowId={(row) => row.id} loading={loading} compact pagination={pagination} onRowClick={onView} onPageChange={onPageChange} onPageSizeChange={onPageSizeChange} rowActions={(row) => <RowActions actions={[{ key: "view", onSelect: () => onView(row) }, ...(canReprocess ? [{ key: "approve" as const, label: "Reprocess", onSelect: () => onReprocess(row) }] : [])]} />} emptyTitle="No biometric logs found" />
);

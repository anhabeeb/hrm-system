...
const BackupCreateDialog = ({ open, loading, error, onOpenChange, onSubmit }: { open: boolean; loading?: boolean; error?: string | null; onOpenChange: (open: boolean) => void; onSubmit: (payload: BackupCreatePayload) => void }) => {
  const [backupType, setBackupType] = useState("metadata");
  const [reason, setReason] = useState("");
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Create backup</DialogTitle><DialogDescription>Create a safe metadata/configuration backup job. Production restore is not executed here.</DialogDescription></DialogHeader><Label className="space-y-1 text-sm">Backup type<Select value={backupType} onValueChange={setBackupType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="metadata">Metadata</SelectItem><SelectItem value="configuration">Configuration</SelectItem><SelectItem value="full_metadata">Full metadata</SelectItem></SelectContent></Select></Label><Textarea placeholder="Reason" value={reason} onChange={(event) => setReason(event.target.value)} />{error ? <InlineAlert title={error} variant="error" /> : null}<DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><LoadingButton loading={loading} onClick={() => onSubmit({ backup_type: backupType, reason })}>Create backup</LoadingButton></DialogFooter></DialogContent></Dialog>;
};
...
const RestoreRequestDialog = ({ open, loading, error, onOpenChange, onSubmit }: { open: boolean; loading?: boolean; error?: string | null; onOpenChange: (open: boolean) => void; onSubmit: (payload: RestoreRequestPayload) => void }) => {
  const [payload, setPayload] = useState<RestoreRequestPayload>({ restore_scope: "metadata_preview", reason: "" });
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Create restore request</DialogTitle><DialogDescription>This screen creates approval requests only. Production restore is not executed from this screen.</DialogDescription></DialogHeader><Label className="space-y-1 text-sm">Backup ID<Input value={payload.backup_id ?? ""} onChange={(event) => setPayload((p) => ({ ...p, backup_id: event.target.value }))} /></Label><Label className="space-y-1 text-sm">Restore scope<Select value={payload.restore_scope} onValueChange={(value) => setPayload((p) => ({ ...p, restore_scope: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="metadata_preview">Metadata preview</SelectItem><SelectItem value="configuration_preview">Configuration preview</SelectItem><SelectItem value="full_restore_placeholder">Full restore placeholder</SelectItem></SelectContent></Select></Label><Textarea placeholder="Reason" value={payload.reason} onChange={(event) => setPayload((p) => ({ ...p, reason: event.target.value }))} />{error ? <InlineAlert title={error} variant="error" /> : null}<DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><LoadingButton loading={loading} onClick={() => onSubmit(payload)}>Request restore</LoadingButton></DialogFooter></DialogContent></Dialog>;
};
...
export const BackupRecoveryPage = () => {
  ...
  const [reasonAction, setReasonAction] = useState<"verify" | "delete" | "approveRestore" | "rejectRestore" | null>(null);
  ...
  const activeTab = tab === "backups" && canViewBackups ? "backups" : tab === "retention" && canManageRetention ? "retention" : tab === "restore" && canViewRestore ? "restore" : "status";
  ...
  const restoreQuery = useQuery({ queryKey: ["backup", "restore", filters], queryFn: () => backupRecoveryApi.listRestoreRequests(filters), enabled: activeTab === "restore" && canViewRestore });
  ...
  const createRestoreMutation = useMutation({ mutationFn: backupRecoveryApi.createRestoreRequest, onSuccess: async () => { setSuccessMessage("Restore request created successfully."); setRestoreOpen(false); await refresh(); } });
  ...
  const activeError = activeTab === "backups" ? backupsQuery.error : activeTab === "retention" ? retentionQuery.error : activeTab === "restore" ? restoreQuery.error : statusQuery.error;
  const mutationError = createBackupMutation.error ?? createRestoreMutation.error ?? retentionMutation.error ?? downloadMutation.error ?? reasonMutation.error;
  return (
    <div>
      <PageHeader title="Backup & Recovery" description="Manage backup jobs, retention, and non-destructive restore approval requests." actions={<div className="flex gap-2">{has("backup.create") ? <Button onClick={() => setBackupOpen(true)}><DatabaseBackup className="h-4 w-4" />Create backup</Button> : null}{has("backup.restore_request") ? <Button variant="outline" onClick={() => setRestoreOpen(true)}><ShieldAlert className="h-4 w-4" />Request restore</Button> : null}</div>} />
      <div className="space-y-4 p-4 md:p-6">
        ...
        <InlineAlert title="Production restore is not executed from this screen." variant="warning">Restore actions create or review requests only. Destructive restore workflows remain backend-controlled.</InlineAlert>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {canViewRestore ? <TabsContent value="restore"><DataTable rows={restoreQuery.data?.data ?? []} loading={restoreQuery.isLoading} pagination={restoreQuery.data?.pagination} columns={[{ key: "id", header: "Request" }, { key: "backup_job_id", header: "Backup" }, { key: "restore_type", header: "Scope", cell: (row) => formatBackupType(row.restore_type) }, { key: "status", header: "Status", cell: (row) => <StatusBadge status={row.status ?? "pending"} /> }, { key: "requested_by", header: "Requested By" }, { key: "created_at", header: "Created", cell: (row) => formatBackupDate(row.created_at) }]} getRowId={(row) => row.id} rowActions={(row) => <RowActions actions={[{ key: "view", onSelect: () => { setSelected(row); setDrawerOpen(true); } }, ...(has("backup.restore_approve") ? [{ key: "approve" as const, onSelect: () => { setSelected(row); setReasonAction("approveRestore"); } }, { key: "reject" as const, onSelect: () => { setSelected(row); setReasonAction("rejectRestore"); } }] : [])]} />} onPageChange={(page) => updateFilters({ page })} onPageSizeChange={(page_size) => updateFilters({ page: 1, page_size })} compact /></TabsContent> : null}
        </Tabs>
      </div>
      ...
      <RestoreRequestDialog open={restoreOpen} loading={createRestoreMutation.isPending} error={createRestoreMutation.error ? friendlyHrmError(createRestoreMutation.error, "Restore request could not be created.") : null} onOpenChange={setRestoreOpen} onSubmit={(payload) => createRestoreMutation.mutate(payload)} />

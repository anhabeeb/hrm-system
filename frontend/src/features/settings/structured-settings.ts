...
export const settingsPageDefinitions: Record<string, SettingsPageDefinition> = {
  organization: {
    ...
    description: "Employee numbering, self-service access, and setup-critical organization controls.",
    group: "company",
    ...
    sections: [
      {
        ...
        setupTarget: "employee-numbering",
        description: "Configure how new employee numbers are generated. Existing employee numbers are preserved unless edited from employee records.",
        settingKey: "employee.numbering",
        fields: [
          { key: "auto_generate_employee_number", label: "Auto-generate employee number", type: "switch", defaultValue: true },
          { key: "prefix", label: "Prefix", type: "text", defaultValue: "EMP" },
          { key: "starting_number", label: "Starting number", type: "number", defaultValue: 1 },
          { key: "number_length", label: "Number length", type: "number", defaultValue: 5 },
          { key: "include_outlet_code", label: "Include outlet code", type: "switch", defaultValue: false },
          { key: "include_department_code", label: "Include department code", type: "switch", defaultValue: false },
          { key: "allow_manual_override", label: "Allow manual override", type: "switch", defaultValue: true },
          { key: "next_employee_number_preview", label: "Next employee number preview", type: "text", defaultValue: "EMP00001", help: "Preview only. The backend still assigns the final number when an employee is created." },
        ],
      },
      {
        title: "Self-Service Settings",
        setupTarget: "self-service-settings",
        description: "Choose what linked employee accounts can view or request. Module-specific options only apply while their parent module is enabled.",
        settingKey: "self_service.default_rules",
        fields: [
          { key: "view_profile", label: "View profile", type: "switch", defaultValue: true },
          { key: "request_kyc_update", label: "Request KYC update", type: "switch", defaultValue: true, help: "Requires the KYC update request feature." },
          { key: "view_attendance", label: "View attendance", type: "switch", defaultValue: true, help: "Requires Attendance Management." },
          { key: "request_attendance_correction", label: "Request attendance correction", type: "switch", defaultValue: true, help: "Requires Attendance Corrections." },
          { key: "request_leave", label: "Request leave", type: "switch", defaultValue: true, help: "Requires Leave Management." },
          { key: "view_payslips", label: "View payslips", type: "switch", defaultValue: true, help: "Requires Payroll Management and Payslips." },
          { key: "view_documents", label: "View documents", type: "switch", defaultValue: true, help: "Requires Document Tracking." },
          { key: "view_assigned_assets", label: "View assigned assets", type: "switch", defaultValue: true, help: "Requires Asset Tracking." },
          { key: "view_issued_uniforms", label: "View issued uniforms", type: "switch", defaultValue: true, help: "Requires Uniform Tracking." },
          { key: "view_contracts", label: "View contracts", type: "switch", defaultValue: true, help: "Requires Contract Tracking." },
          { key: "change_password", label: "Change password", type: "switch", defaultValue: true },
          { key: "enable_2fa", label: "Enable 2FA", type: "switch", defaultValue: true },
        ],
      },
    ],
  },
  approvals: {
    ...
    description: "Configure module-aware approval workflow behavior. Finance steps are optional and appear only when configured.",
    group: "approval_workflows",
    endpointPath: "approval_workflows",
    managePermission: "approval_workflows.manage",
    sections: [
      {
        title: "Approval Workflow Defaults",
        setupTarget: "approval-workflows",
        description: "Review workflow names, module/request types, step ordering, approver routing, and active status.",
        settingKey: "approvals.default_rules",
        fields: [
          { key: "approval_workflows_enabled", label: "Approval workflows enabled", type: "switch", defaultValue: true },
          { key: "workflow_name", label: "Workflow name", type: "text", defaultValue: "Default approval workflow" },
          { key: "request_module_type", label: "Request/module type", type: "text", defaultValue: "leave_request" },
          { key: "step_order", label: "Step order", type: "number", defaultValue: 1 },
          { key: "approver_type", label: "Approver type", type: "select", options: [
            ...
            { label: "HR group", value: "hr_group" },
            { label: "Finance group", value: "finance_group" },
            { label: "Role", value: "role" },
            { label: "Level", value: "level" },
          ] },
          { key: "approver_role", label: "Approver role", type: "text" },
          { key: "approver_level", label: "Approver level", type: "text" },
          { key: "department_head_enabled", label: "Department head", type: "switch", defaultValue: true },
          { key: "direct_manager_enabled", label: "Direct manager", type: "switch", defaultValue: true },
          { key: "specific_user_enabled", label: "Specific user", type: "switch", defaultValue: false },
          { key: "hr_group_enabled", label: "HR group", type: "switch", defaultValue: true },
          { key: "finance_group_enabled", label: "Finance group", type: "switch", defaultValue: false, help: "Finance approval is optional." },
          { key: "workflow_active", label: "Active/inactive status", type: "switch", defaultValue: true },
        ],
      },
    ],
  },
  ...
  attendance: {
    ...
    sections: [
      ...
      },
      {
        ...
        description: "Duty Roster module availability is controlled from the Module Availability section on the Duty Roster settings page; these settings only tune roster/attendance interaction while enabled.",
    ...
    sections: [
      {
        ...
        description: "Leave Management availability is controlled from the Module Availability section on this settings page; these settings tune the enabled leave workflow.",
      ...
      },
      {
        ...
        description: "Long Leave Management is enabled from the Module Availability section on this settings page and depends on Leave Management.",
...
export const additionalSettingsPageDefinitions: Record<string, SettingsPageDefinition> = {
    ...
    sections: [
      {
        ...
        description: "Document Tracking availability is controlled from the Module Availability section on the Documents settings page; these fields tune the enabled document workflow.",
      ...
      },
      {
        ...
        description: "Contract Tracking module availability is controlled from the Module Availability section on the Contract settings page. Contract document upload also requires Document Tracking.",
  ...
  assets: {
    ...
    description: "Configure how company assets are issued, returned, confirmed, and handled when lost or damaged.",
    group: "assets_uniforms",
    endpointPath: "assets_uniforms",
    managePermission: "assets_settings.manage",
    parentFeatureKey: "asset_tracking",
    parentFeatureLabel: "Asset Tracking",
    sections: [
      {
        title: "Asset Issue Rules",
        setupTarget: "asset-issue-rules",
        description: "These controls apply when Asset Tracking is enabled. Existing asset assignment records are preserved if the module is disabled.",
        settingKey: "assets.issue_rules",
        fields: [
          { key: "require_approval_before_issuing_asset", label: "Require approval before issuing asset", type: "switch", defaultValue: false },
          { key: "require_return_confirmation", label: "Require return confirmation", type: "switch", defaultValue: true },
          { key: "track_asset_condition", label: "Track asset condition", type: "switch", defaultValue: true },
          { key: "require_employee_signature", label: "Require employee signature", type: "switch", defaultValue: false },
          { key: "require_manager_confirmation", label: "Require manager confirmation", type: "switch", defaultValue: false },
          { key: "deduction_for_lost_damaged_asset", label: "Deduction for lost/damaged asset", type: "switch", defaultValue: false },
          { key: "return_due_date_behavior", label: "Return due date behavior", type: "select", options: [
            { label: "Optional", value: "optional" },
            { label: "Required for temporary issue", value: "required_for_temporary_issue" },
  ...
  uniforms: {
    ...
    description: "Configure how uniforms are sized, issued, returned, replaced, and handled when lost or damaged.",
    group: "assets_uniforms",
    endpointPath: "assets_uniforms",
    managePermission: "assets_settings.manage",
    parentFeatureKey: "uniform_tracking",
    parentFeatureLabel: "Uniform Tracking",
    sections: [
      {
        title: "Uniform Issue Rules",
        setupTarget: "uniform-issue-rules",
        description: "These controls apply when Uniform Tracking is enabled. Existing uniform issue records are preserved if the module is disabled.",
        settingKey: "uniforms.issue_rules",
        fields: [
          { key: "size_tracking_enabled", label: "Size tracking enabled", type: "switch", defaultValue: true },
          { key: "quantity_tracking", label: "Quantity tracking", type: "switch", defaultValue: true },
          { key: "return_required", label: "Return required", type: "switch", defaultValue: true },
          { key: "replacement_cycle", label: "Replacement cycle", type: "select", options: [
          ...
          { key: "require_employee_signature", label: "Require employee signature", type: "switch", defaultValue: false },
          { key: "require_manager_confirmation", label: "Require manager confirmation", type: "switch", defaultValue: false },
          { key: "deduction_for_lost_damaged_uniform", label: "Deduction for lost/damaged uniform", type: "switch", defaultValue: false },
          { key: "issue_approval_required", label: "Issue approval required", type: "switch", defaultValue: false },
          { key: "return_approval_required", label: "Return approval required", type: "switch", defaultValue: false },
        ],
      },
    ],
  },

...
export const RostersPage = () => {
  ...
  return (
    <div>
      {(canRequestRosterChange || canCreateRoster || canBulkRoster || canCreateShiftTemplate) ? <PageActionBar label="Duty rosters page actions"><div className="flex flex-wrap items-center justify-end gap-2">{canRequestRosterChange ? <Button onClick={() => setChangeRequestOpen(true)} variant="outline" data-setup-target="roster-approvals">Request change</Button> : null}{canCreateRoster ? <Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" /> Create shift</Button> : null}{canBulkRoster ? <Button variant="outline" onClick={() => setBulkOpen(true)}>Bulk create</Button> : null}{canCreateShiftTemplate ? <Button variant="outline" onClick={() => setTemplateOpen(true)} data-setup-target="shift-templates">New template</Button> : null}</div></PageActionBar> : null}

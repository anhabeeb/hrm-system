...
export const DepartmentsPage = () => {
  ...
  const mutation = useMutation<unknown, unknown, { id?: string; values: DepartmentFormValues }>({
    ...
    onSuccess: async (_, variables) => { setSuccess(variables.id ? "Department updated successfully." : "Department created successfully."); setError(null); setFormOpen(false); await queryClient.invalidateQueries({ queryKey: ["departments"] }); },
  ...
  const canCreate = auth.hasPermission("departments.create");
    ...
    <div>
      ...
      <div className="space-y-4 p-4 md:p-6">
        ...
        <div className="flex flex-col gap-3 rounded-lg border bg-card p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div><h2 className="text-base font-semibold">Departments</h2><p className="text-sm text-muted-foreground">Compact table-first administration.</p></div>
          {canCreate ? <Button onClick={() => { setSelected(null); setError(null); setFormOpen(true); }}><BriefcaseBusiness className="h-4 w-4" /> Create Department</Button> : null}
        </div>
        ...
        <DataTable compact loading={query.isLoading} rows={query.data?.data ?? []} pagination={query.data?.pagination} onPageChange={(page) => setFilterValues({ page })} onPageSizeChange={(page_size) => setFilterValues({ page: 1, page_size })} getRowId={(row) => row.id} onRowClick={(row) => { setSelected(row); setDrawerOpen(true); }} emptyTitle="No departments found." columns={[
          { key: "name", header: "Department Name" }, { key: "code", header: "Code", cell: (row) => row.code ?? "Not set" }, { key: "status", header: "Status", cell: (row) => <StatusBadge status={row.status} /> }, { key: "employees", header: "Employees Count", cell: () => "Available in reports" },
